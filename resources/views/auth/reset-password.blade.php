...
<head>
    @include('partials.public-head', [
        'pageTitle' => 'Set New Password - PT Sistem Jaya Abadi',
        'seoTitle' => 'Set New Password - PT Sistem Jaya Abadi',
        'seoDescription' => 'Configure a new secure password for the PT Sistem Jaya Abadi management portal.',
        'seoUrl' => route('password.reset', ['token' => $request->route('token')]),
    ])
</head>
    ...
    <div class="relative min-h-screen w-full flex bg-[#FAF9F6] text-primary antialiased">
            ...
            <div class="flex items-center justify-between">
                <a href="/" class="flex items-center gap-3 group">
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="text-[9px] uppercase tracking-[0.2em] font-bold text-muted">Security Console</span>
                    </div>
                </a>

                <a href="{{ route('login') }}" class="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-muted hover:text-primary transition-colors">
            ...
            <div class="w-full max-w-md mx-auto my-auto py-8">
                ...
                <div class="mb-8">
                    ...
                    <p class="text-xs sm:text-sm text-muted leading-relaxed">
                        Configure a robust, unique password to secure your administrative operations workstation.
                    </p>
                </div>
                ...
                @if ($errors->any())
                    <div class="mb-6 p-4 rounded-2xl bg-secondary/10 border border-secondary/20 flex items-start gap-3 shadow-sm">
                        ...
                        <div class="text-xs font-semibold text-secondary leading-relaxed">
                            <p>Unable to update credentials. Please ensure both passwords match and satisfy length requirements.</p>
                        </div>
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('password.store') }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <div class="relative flex items-center">
                            ...
                            <input class="w-full bg-white border {{ $errors->has('email') ? 'border-secondary focus:ring-secondary focus:border-secondary' : 'border-black/10 focus:ring-secondary focus:border-secondary' }} text-primary text-xs sm:text-sm font-medium rounded-xl py-3.5 pl-11 pr-4 focus:outline-none focus:ring-2 focus:ring-secondary/20 shadow-sm transition-all placeholder:text-muted/60"
                                   id="email" name="email" value="{{ old('email', $request->email) }}" type="email" required autofocus autocomplete="username" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative flex items-center">
                            ...
                            <input class="w-full bg-white border {{ $errors->has('password') ? 'border-secondary focus:ring-secondary focus:border-secondary' : 'border-black/10 focus:ring-secondary focus:border-secondary' }} text-primary text-xs sm:text-sm font-medium rounded-xl py-3.5 pl-11 pr-11 focus:outline-none focus:ring-2 focus:ring-secondary/20 shadow-sm transition-all placeholder:text-muted/60"
                                   id="password" name="password" placeholder="Minimum 8 characters" type="password" required autocomplete="new-password" />
                            <button type="button" onclick="toggleVisibility('password', 'toggle-pass-icon')" aria-label="Toggle password visibility"
                                ...
                                <span id="toggle-pass-icon" class="material-symbols-outlined text-lg leading-none">visibility</span>
                            </button>
                        </div>
                    </div>
                    ...
                    <!-- Confirm Password -->
                    <div>
                        <label class="block text-[10px] font-bold text-muted uppercase tracking-[0.2em] mb-2" for="password_confirmation">
                            Confirm New Password
                        </label>
                        <div class="relative flex items-center">
                            <span class="material-symbols-outlined absolute left-4 text-muted text-lg pointer-events-none">verified_user</span>
                            <input class="w-full bg-white border {{ $errors->has('password_confirmation') ? 'border-secondary focus:ring-secondary focus:border-secondary' : 'border-black/10 focus:ring-secondary focus:border-secondary' }} text-primary text-xs sm:text-sm font-medium rounded-xl py-3.5 pl-11 pr-11 focus:outline-none focus:ring-2 focus:ring-secondary/20 shadow-sm transition-all placeholder:text-muted/60"
                                   id="password_confirmation" name="password_confirmation" placeholder="Repeat new password" type="password" required autocomplete="new-password" />
                            <button type="button" onclick="toggleVisibility('password_confirmation', 'toggle-confirm-icon')" aria-label="Toggle confirmation visibility"
                                class="absolute right-3.5 top-1/2 -translate-y-1/2 text-muted hover:text-primary transition-colors focus:outline-none p-1 rounded-lg">
                                <span id="toggle-confirm-icon" class="material-symbols-outlined text-lg leading-none">visibility</span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="pt-2 flex flex-col gap-3">
                        <button class="w-full bg-secondary hover:bg-secondary-hover text-white py-4 px-6 rounded-xl font-bold text-xs uppercase tracking-wider shadow-glow active:scale-[0.98] transition-all duration-300 flex items-center justify-center gap-2 group cursor-pointer"
                                type="submit">
                            <span>Update Credentials &amp; Proceed</span>
                        ...
                        <a href="{{ route('login') }}" class="w-full py-3 text-center text-xs font-bold uppercase tracking-wider text-muted hover:text-primary transition-colors flex items-center justify-center gap-1.5">
                            <span class="material-symbols-outlined text-sm">close</span>
                            <span>Cancel and Return to Portal</span>
                        </a>
                    </div>
                </form>
            </div>
        ...
        <div class="hidden lg:block lg:w-1/2 relative bg-primary overflow-hidden min-h-screen">
            ...
            <div class="absolute inset-0">
                <img src="{{ asset('assets/hero_villa.png') }}" alt="Architecture Cover"
                    class="w-full h-full object-cover opacity-75 filter contrast-105 scale-105"
                    onerror="this.src='https://placehold.co/1200x1600/11161B/FAF9F6?text=SJA+Institutional+Security'">
            </div>
            ...
            <div class="absolute bottom-12 left-12 right-12 z-10">
                <div class="glass-card bg-primary/80 backdrop-blur-xl border border-white/15 p-8 rounded-[2rem] shadow-2xl text-white">
                    ...
                    <h2 class="font-display text-2xl font-bold tracking-tight text-white mb-2">
                        Setting Strong Cryptographic Defense.
                    </h2>
                    
                    <p class="text-xs text-white/70 leading-relaxed mb-6 max-w-lg">
                        Passwords are transformed via irreversible one-way hashing with adaptive cost factors, protecting institutional data against unauthorized penetration.
                    </p>

                    <!-- Credibility Metrics Bar -->
                    <div class="grid grid-cols-3 gap-4 pt-4 border-t border-white/10 text-center">
                        <div>
                            <span class="font-display font-extrabold text-lg text-white block">12+</span>
                            <span class="text-[9px] uppercase font-bold tracking-wider text-white/60">Hash Rounds</span>
                        </div>
                        <div class="border-x border-white/10">
                            <span class="font-display font-extrabold text-lg text-secondary block">Zero</span>
                            <span class="text-[9px] uppercase font-bold tracking-wider text-white/60">Plaintext Stored</span>
                        </div>
                        <div>
                            <span class="font-display font-extrabold text-lg text-emerald-400 block">100%</span>
                            <span class="text-[9px] uppercase font-bold tracking-wider text-white/60">Compliance</span>
                        </div>
                    </div>
                </div>
            </div>
