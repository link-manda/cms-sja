...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form id="project-form" method="POST" action="{{ route('projects.update', $project->id) }}" enctype="multipart/form-data" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="p-6 bg-default-50 border border-default-200 rounded-lg mt-6 mb-6">
                    ...
                    <div class="mb-4 flex items-center">
                        ...
                        @php
                            $isPromotionActive = session()->hasOldInput() ? (bool) old('is_for_sale_or_rent') : (bool) $project->is_for_sale_or_rent;
                        @endphp
                        <input type="checkbox" id="is_for_sale_or_rent" name="is_for_sale_or_rent" value="1"
                            class="w-5 h-5 text-primary rounded border-default-300 cursor-pointer" {{ $isPromotionActive ? 'checked' : '' }}>
                    ...
                    <div id="promotion-fields-container" class="grid grid-cols-1 md:grid-cols-2 gap-4 {{ $isPromotionActive ? '' : 'hidden' }}">
                        <!-- Property Type -->
                        <div>
                            ...
                            <select id="property_type" name="property_type"
                                ...
                                <option value="Investment" {{ old('property_type', $project->property_type) == 'Investment' ? 'selected' : '' }}>For Investment (Commercial Property)</option>
                                <option value="Rent" {{ old('property_type', $project->property_type) == 'Rent' ? 'selected' : '' }}>For Rent (Boarding/Kos / Villa)</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" id="price" name="price" placeholder="Example: 1500000" value="{{ old('price', (int)$project->price ?: '') }}"
                                class="form-input">
                        ...
                        @php
                            $currentPropertyType = old('property_type', $project->property_type);
                            $isRoiVisible = $isPromotionActive && in_array($currentPropertyType, ['Sale', 'Investment']);
                            $isKosVisible = $isPromotionActive && $currentPropertyType === 'Rent';
                        @endphp
                        ...
                        <div id="roi-estimation-wrapper" class="md:col-span-2 {{ $isRoiVisible ? '' : 'hidden' }}">
                            ...
                            <textarea id="roi_estimation" name="roi_estimation" rows="3"
                                ...
                                class="form-input">{{ old('roi_estimation', $project->roi_estimation) }}</textarea>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (addVideoBtn && videoContainer) {
                ...
                addVideoBtn.addEventListener('click', () => {
                    ...
                    if (groups.length + existingGalleryCount + persistedTempCount + galleryFilesCount >= 10) {
            ...
            if (projectForm) {
                projectForm.addEventListener('submit', function(e) {
                    ...
                    const totalItems = existingGalleryCount + persistedTempCount + galleryFiles.length + activeVideoLinks.length;
                    if (totalItems > 10) {
                        e.preventDefault();
                        alert(`Total gallery quota is max 10 items (photos and videos combined). Currently proposed: ${totalItems} items (${existingGalleryCount} existing, ${persistedTempCount + galleryFiles.length} photos, ${activeVideoLinks.length} videos). Please reduce items.`);
                        return;
                    }
                    ...
                    function uploadNextFile() {
                        if (currentIndex >= totalFiles) {
                            if (modalStatusText) modalStatusText.textContent = 'Menyimpan perubahan proyek...';
